...
class State {
	...
	constructor( settings = {} ) {
		...
		this._volumeThreshold = ( settings.volumeThreshold || 10 );
		this._failureThreshold = ( settings.failureThreshold || FIVE_PERCENT );
		this._activeThreshold = ( settings.activeThreshold || 100 );
		...
		// In addition to tracking errors, this state implementation also tracks 
		// concurrent requests. If the number of concurrent requests exceeds the given
		// threshold (activeThreshold), the circuit will open until some of the pending
		// requests have completed.
		this._activeRequestCount = 0;

		// Keep track of the pending failed count just to ensure proper state.
	...
	// request to pass through the circuit despite its unhealthy state).
	...
	// I determine if the circuit is currently opened (and unable to accept requests).
	isOpened() {
	...
	// I track new requests being routed through the circuit breaker.
	trackEmit() {
	...
	// I track requests that have failed to execute in the circuit breaker.
	trackFailure( duration, error ) {

		this._assertActiveRequestCount();

		this._activeRequestCount--;
		// Not all errors indicate an unhealthy resource. For example, a "Not Found"
		// error, returned from an API, relates only to the content of the request and
		// not to the health of a API. As such, some errors should be classified as a
		// "success" metric.
		this._isFailure( error )
	...
	// I track new requests coming into the circuit.
	trackFallbackEmit() {
	...
	_assertActiveRequestCount() {
		...
		if ( this._activeRequestCount <= 0 ) {
			...
			throw( new StateError( "You cannot track the end of an execution when you have no pending requests." ) );
	...
	_isHealed() {
		...
		// success event.
		return( ( totalCount === 1 ) && ( totalCount === successCount ) );
